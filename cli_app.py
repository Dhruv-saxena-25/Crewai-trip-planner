...
class TripCrew:
    ...
    def run(self):
        try:
            agent = TripAgents(llm = self.llm)
            tasks = TripTasks()
            
            city_selector_agent = agent.city_selection_agent()
            local_expert_agent = agent.local_expert()
            travel_concierge_agent = agent.travel_concierge()
